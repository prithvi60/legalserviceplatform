...
import LetterPreview, { renderStep } from "@/components/Layout/LetterPreview";
...
// const storageKey = "nda-form-data";
...
// const fileName = "nda-agreement.pdf";

// const url = "/documentation/business_contracts";
...
const NDAPreview: React.FC<DocumentPreviewProps> = ({ documentType }) => {
    ...
    const { title, initialFormData, fieldGroups, sensitiveContent, documentSlug, fileName, storageKey, url } = config;
    ...
    const { data: GetDocType } = useQuery<GetBFResponse>(GET_BUSINESS_FORMS, {
        variables: { userId, DocType: documentSlug, orderBy: { DocNumber: "desc" } },
    });
    ...
    const [createBusinessForm] = useMutation(CREATE_BUSINESS_FORM, {
        refetchQueries: [
            { query: GET_BUSINESS_FORMS, variables: { userId, DocType: documentSlug } },
        ],
    });
    ...
    const handleInputChange = useCallback(
        (name: keyof typeof initialFormData, value: string) => {
            setState((prev) => ({
                ...prev,
    ...
    const saveProgressToDatabase = useCallback(async () => {
        ...
        const input = {
            userId,
            DocType: resume ? docType : documentSlug,
            formData: currentData,
            status:
                (resume ? currentData.progress : state.progress) === 100
                    ? "IsComplete"
                    : "IsPending",
        };
        ...
        const existingForm = resume
            ...
            : GetDocType?.getBusinessForms
                ?.filter((form) => form.DocType === input.DocType && form.userId === userId)
        ...
        try {
            ...
            if (existingForm) {
                console.log("Updating existing form with DocNumber:", existingForm.DocNumber);
            ...
            toast.error("Failed to submit the form. Please check your internet connection and reload or try again later.", {
                position: "top-right",
                duration: 3000,
                style: {
                    border: "1px solid #EB1C23",
    ...
    }, [state.step, isCurrentStepComplete, saveProgressToDatabase, currentData, fieldGroups.length, storageKey]);
    ...
    }, [resume, GetDoc?.getBusinessForm, docNumber, docType, getDocLoading, storageKey, initialFormData]);
    ...
    useEffect(() => {
        // console.log("Checking conditions for download...");
        // console.log("paymentStatus:", paymentStatus);
        // console.log("isDownloading:", state.isDownloading);
        // console.log("Business Forms:", GetDocType?.getBusinessForms);
        if (
            ...
            const performDownload = async () => {
                try {
                    ...
                    window.location.replace(`${url}/${documentSlug}`);
                } catch (error) {
    ...
    }, [paymentStatus, state.isDownloading, GetDocType, toPDF, storageKey, url, documentSlug]);
