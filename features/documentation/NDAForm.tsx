...
import React, { useState, ChangeEvent, JSX, useEffect, useMemo, useCallback } from "react";
...
import LetterPreview from "@/components/Layout/LetterPreview";
...
const NDAPreview: React.FC = () => {
    ...
    const [state, setState] = useState(() => {
        ...
        return {
            ...
            isFinished: false,
            encryptedContent: encryptText(SENSITIVE_CONTENT),
            isDecrypted: false,
            isDownloading: false,
    ...
    const { data: RoleBased, loading: userLoading } = useQuery<GetUserResponse>(GET_USER, {
    ...
    const { data: GetDoc, loading: getDocLoading } = useQuery<GetOBFResponse>(GET_BUSINESS_FORM, {
        variables: { userId, DocType: docType, DocNumber: docNumber },
    });

    const [createBusinessForm] = useMutation(CREATE_BUSINESS_FORM, {
        refetchQueries: [{ query: GET_BUSINESS_FORMS, variables: { userId, DocType: "NDA" } }],
    });
    ...
    const totalFields = useMemo(() => fieldGroups.reduce((acc, group) => acc + group.fields.length, 0), []);

    const calculateProgress = useCallback(() => {
        const filledFields = Object.entries(state.formData).filter(([key, value]) => {
            const fieldStep = fieldGroups.find((group) => group.fields.includes(key as keyof FormData2))?.step;
            return fieldStep && fieldStep <= state.step && value !== "";
        }).length;
        return Math.floor((filledFields / totalFields) * 100);
    }, [state.formData, state.step, totalFields]);

    const isCurrentStepComplete = useCallback(() => {
        const currentGroup = fieldGroups.find((group) => group.step === state.step);
        return currentGroup ? currentGroup.fields.every((field) => state.formData[field] !== "") : false;
    }, [state.step, state.formData]);

    const handleInputChange = useCallback((name: keyof FormData2, value: string) => {
    ...
    const saveProgressToDatabase = useCallback(async () => {
        ...
        const input = {
            userId,
            DocType: resume ? docType : "NDA",
            formData: currentData,
            status: (resume ? currentData.progress : state.progress) === 100 ? "IsComplete" : "IsPending"
        };

        // Get existing form based on document type
        const existingForm = resume
    ...
    }, [userId, state, GetDocType, currentData, GetDoc?.getBusinessForm, docType, resume, createBusinessForm, updateBusinessForm]);
    ...
    const handleNext = useCallback(async () => {
        if (state.step < fieldGroups.length && isCurrentStepComplete()) {
            ...
            sessionStorage.setItem(
                ...
                JSON.stringify({
                    ...currentData,
                    step: (prev: number) => prev + 1,
                })
            );
            await saveProgressToDatabase();
        }
    }, [state.step, isCurrentStepComplete, saveProgressToDatabase, currentData]);
    ...
    const handleDownload = useCallback(async () => {
        ...
        const paymentUrl = `/payment?amount=5.99&redirect=${encodeURIComponent(currentPath)}`;
    ...
    }, [currentData, state.formData, state.step, state.progress]);
        ...
        const injectDataToSessionStorage = () => {
            ...
            if (documentData) {
                ...
                if (docNumber && docType) {
                    ...
                    setState((prev) => ({ ...prev, step: dbStep, progress: dbProgress, formData: { ...initialFormData, ...dbFormData } }));
                }
            }
        };
    ...
    useEffect(() => {
        if (paymentStatus === "success" && !state.isDownloading && GetDocType?.getBusinessForms?.length) {
            const performDownload = async () => {
                try {
                    setState((prev) => ({ ...prev, isDownloading: true, isDecrypted: true }));
    ...
    }, [paymentStatus, state.isDownloading, GetDocType, toPDF]);

    const isFormComplete = (): boolean => {
        return state.isFinished && state.progress === 100 || state.progress === 100 && GetDoc?.getBusinessForm.status === "IsComplete";
    };
    ...
    const renderStep = ({ step, formData, handleInputChange }: { step: number; formData: FormData2; handleInputChange: (name: keyof FormData2, value: string) => void }): JSX.Element | null => {
        switch (step) {
            case 1:
                return (
            ...
            <div className="flex flex-col lg:flex-row justify-center items-start gap-10">
                {!isFormComplete() && (
                    <div className="w-full basis-full lg:basis-2/5">
                        <Card className="lg:max-w-md">
                            <CardBody>
                                <Progress value={state.progress} className="mb-4" color="primary" showValueLabel />
                                {renderStep({ step: state.step, formData: state.formData, handleInputChange })}
                ...
                <div className="w-full basis-full lg:basis-3/5 select-none">
                    <Card className="w-full">
                        <CardBody>
                            <h3 className="text-lg font-semibold mb-4 text-[#1E318D]">Preview</h3>
