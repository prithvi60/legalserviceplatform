...
const NDAPreview: React.FC = () => {
    ...
    // console.log("GET", GetDocType?.getBusinessForms?.[0]);


    // Load data from the database or session storage
    useEffect(() => {
        const documentData = GetDocType?.getBusinessForms?.[0]?.formData;
        const injectDataToSessionStorage = () => {
            if (resume && documentData) {
                ...
                // Inject database data into sessionStorage
    ...
    }, [resume, GetDocType?.getBusinessForms, getDocLoading]);
        ...
        const performDownload = async () => {
            if (
                ...
                try {
                    ...
                    // Safely access the business form data
                    const businessForm = GetDocType?.getBusinessForms?.[0];

                    if (businessForm?.DocNumber) {
                        await deleteBusinessForm({
                            variables: {
                                input: {
                                    userId: userId,
                                    DocType: "NDA",
                                    DocNumber: businessForm?.DocNumber,
                                },
                            },
                        });
                        // if (deleteResponse?.deleteBusinessForm) {
                        //     alert("Form deleted successfully!");
                        // } else {
                        //     console.error("Failed to delete business form");
                        //     alert("Form deletion failed!");
                        //     throw new Error("Failed to delete business form");
                        // }
                    }
    ...
    const saveProgressToDatabase = async (): Promise<void> => {
        ...
        try {
            ...
            //     (form) => form.DocType === input.DocType && form.DocNumber === GetDocType?.getBusinessForms?.[0].DocNumber
            // );
            // console.log("existingForm", existingForm);

            const existingForm = GetDocType?.getBusinessForms?.[0];
                                    ...
                                    {JSON.parse(sessionStorage.getItem(STORAGE_KEY) || "{}")
                                        ...
                                        progress === 100 && (
                                            <Button
                                                ...
                                                color="warning"
                                                radius="sm"
                                                className="text-white animate-pulse hover:scale-110 transition-all transform duration-400 ease-in-out"
                                            >
                                                Finish!
                                            </Button>
                                        )}
