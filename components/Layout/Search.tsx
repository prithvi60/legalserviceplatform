...
const Search = () => {
    return (
        <section className="padding w-full space-y-10">
            <Input
                isRequired
                variant="bordered"
                startContent={
                ...
                type="text"
                classNames={{
                    base: "border-[#666666] rounded-full",
                    input: "placeholder:!text-[#666666] !text-black",
                    innerWrapper: "bg-transparent",
                    inputWrapper:
                        "!bg-transparent data-[hover=true]:!bg-white/10 group-data-[focus=true]:!bg-white/10 group-data-[focus=true]:!text-black !cursor-text",
                    helperWrapper: "!text-[#f26161]",
                    errorMessage: "!text-[#f26161]",
                }}
                className="bg-transparent rounded-lg text-black"
            />
            <div className="flex mx-auto items-center no_scrollbar gap-5 xl:gap-10  w-full overflow-x-scroll bg-white">
                {communityServices.map((item, idx) => (
                    <Card
                        key={idx}
                        className="py-4 px-6 border-2 border-warning linear-yellow_1 min-w-64 mx-auto md:min-w-72"
                    >
                        <CardHeader className="pb-3 pt-2 px-4 flex-col items-start">
                            <div className="flex flex-col">
                                <h4 className="text-lg md:text-xl font-Archivo font-bold tracking-wider">
                                    {item.title}
                                </h4>
                            ...
                            <ul className="space-y-4">
                                {item.lists.map((list, id) => (
                                    ...
                                    >
                                        <Link
                                            href={"#"}
                                            className="w-4/5 pb-2 border-b-2 border-black"
                                        >
                                            <h4>{list}</h4>
                                        </Link>
